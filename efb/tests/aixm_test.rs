...
fn parse_aixm_navigation_data() {
    ...
    assert!((bor.coordinate().y() - 52.368389).abs() < 0.001);

    // Airspace: DONLON CTR should contain the airport
    let inside = geo::Point::new(-31.94, 52.36); // (lon, lat)
    let airspaces = nd.at(&inside, efb::measurements::Length::nm(0.0));
    assert!(!airspaces.airspaces.is_empty());
    assert_eq!(airspaces.airspaces[0].name, "DONLON CTR");
}
