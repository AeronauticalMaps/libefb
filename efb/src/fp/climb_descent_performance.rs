...
/// [`vertical_rate`] is the rate of climb; in a descent table it is the rate
/// of descent. The value is always positive — direction is implied by the
/// context in which the table is used.
///
/// [`level`]: ClimbDescentBand::level
/// [`vertical_rate`]: ClimbDescentBand::vertical_rate
///
/// # Example
///
/// ```
...
pub struct ClimbDescentResult {
    ...
    pub fuel: Fuel,
    /// Horizontal distance covered at TAS (no wind correction).
    ///
    /// After calling [`with_wind`], this field holds the wind-corrected
    /// ground distance instead.
    ///
    /// [`with_wind`]: ClimbDescentResult::with_wind
...
impl ClimbDescentResult {
    /// Returns a copy with [`horizontal_distance_tas`] corrected for wind.
    ///
    ...
    ///
    /// Derive the headwind value from [`Wind::headwind`] with the leg bearing.
    ///
    /// [`horizontal_distance_tas`]: ClimbDescentResult::horizontal_distance_tas
...
impl ClimbDescentPerformance {
    ...
    pub fn between(
        ...
        Some(ClimbDescentResult {
            time: total_time,
            fuel,
            horizontal_distance_tas: total_dist,
        })
    }
}
...
mod tests {
    use super::*;
    use crate::measurements::{LengthUnit, Mass, VerticalRateUnit};
    use crate::FuelType;
