...
impl FuelPlanning {
    pub fn new(
        ...
        let climb = climb_perf.and_then(|cp| {
            ...
            let first_leg_headwind = route
                .legs()
                .first()
                .and_then(|leg| leg.wind().zip(Some(leg.bearing())))
                .map(|(wind, bearing)| wind.headwind(bearing))
                .unwrap_or(crate::measurements::Speed::kt(0.0));
            Some(
                cp.compute(&origin_elev, &cruise_level)?
                    .with_wind(first_leg_headwind)
                    .fuel,
            )
        });
        ...
        let descent = descent_perf.and_then(|dp| {
            ...
            let last_leg_headwind = route
                ...
                .unwrap_or(crate::measurements::Speed::kt(0.0));
            Some(
                dp.compute(&dest_elev, &cruise_level)?
                    .with_wind(last_leg_headwind)
                    .fuel,
            )
        });
